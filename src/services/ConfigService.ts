...
const DEFAULT_CONFIG: AppConfig = { registryUrl: "https://registry.kiln.dev" };
...
export class ConfigService {
  ...
  public async setValue(key: string, value: string): Promise<void> {
    if (!/^[A-Za-z][A-Za-z0-9_-]*$/.test(key)) throw new KilnError("Configuration keys must use letters, numbers, underscores, or hyphens.");
  ...
  private async readJson<T>(path: string, fallback: T): Promise<T> {
    ...
    catch (error) { throw new KilnError(`Unable to read ${path}; ensure it contains valid JSON.`, error); }
  }
  private async writeJson(path: string, value: object): Promise<void> {
    try { await writeFile(path, `${JSON.stringify(value, null, 2)}\n`, "utf8"); }
    catch (error) { throw new KilnError(`Unable to write ${path}.`, error); }
  }
}
